...
class AnalyzeFacture:
    ...
    def _extract_infos_facture(self, joined_text: str) -> dict:
        ...
        if match:
            groups = {
                "numero_facture": match.group(1) if match.lastindex >= 1 else "",
                "date_facture": match.group(2) if match.lastindex >= 2 else "",
            }
            client_match = re.search(r"N.?\s*client\s*:?\s*([A-Z0-9-]+)", joined_text, flags=re.IGNORECASE)
            if client_match:
                groups["numero_client"] = client_match.group(1)

        return self.build_entry(block_config.get("description"), pattern, match.group(0) if match else None, groups)

    def _extract_lignes_facture(self, rec_texts: list[str]) -> dict:
        block_config = self.config["bloc_lignes_facture"]
        start_index = self.find_token_index(rec_texts, r"(Description|D[ée]signation|Article|Libell[ée])")
        end_index = None
    ...
    def _extract_conditions_paiement(self, joined_text: str) -> dict:
        ...
        if match:
            groups = {
                "conditions_paiement": match.group(1) if match.lastindex >= 1 else "",
                "mode_paiement": match.group(2) if match.lastindex >= 2 else "",
            }
